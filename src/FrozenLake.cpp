...
std::vector<Action> FrozenLakeEnv::getPossibleActions(const GameState &state) const {
    ...
    auto &stateTransitions = it->second; //get value = map of possible actions
...
std::set<GameState> FrozenLakeEnv::getAllStates() {
    return m_allStates;
}
...
FrozenLakeEnv::init(std::string name, vec2DGameState map, std::map<std::string, double> reward,
    ...
    for (size_t row = 0; row < m_map.size(); ++row) {
        for (size_t col = 0; col < m_map[row].size(); ++col) {
            ...
            m_allStates.insert(currentState);
            if (stateName == 'S') {
            ...
            for (size_t i = 0; i < nextStates.size(); ++i) {
                ...
                m_rewards[nextStates[i].second] = currentReward;
...
double FrozenLakeMDP::getCurrentReward(const GameState &state) const {
	auto stateIt = m_rewards.find(state);
